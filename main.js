...
WebVR.Util = {
    ...
    sphere2world: function (h, v) {
        if (h !== undefined && v !== undefined) {
            var resVector = new THREE.Vector3();
            WebVR.Util.spherical.theta = h / 180 * Math.PI;
            WebVR.Util.spherical.phi = v / 180 * Math.PI;
            WebVR.Util.spherical.radius = 1000;
            resVector.setFromSpherical(WebVR.Util.spherical);
            return resVector;
        }
        return null;
    }
...
WebVR.run = function (containerId, dataJsonUrl, control) {
    WebVR.container = document.getElementById(containerId);
    ...
    this.setAutoRotate = function (value, seconds) {
        if (value) {
            seconds = seconds || 10000;
            controls.autoRotate = true;
            intervalAutoRotate = setInterval(function () {
                enterNextScene();
            }, seconds);
        }
        else {
            clearInterval(intervalAutoRotate);
    ...
    this.addLabel = function (labelObj, sceneObjId) {
        var sceneObj;
        if (sceneObjId) {
            sceneObj = getSceneById(sceneObjId);
        }
        else {
    ...
    this.resetLabelByLabelObj = function (labelObj) {
        if (labelObj && labelObj.location) {
            var vec3 = WebVR.Util.sphere2world(labelObj.location.h, labelObj.location.v);
            labelObj.location.x = -vec3.x;
            labelObj.location.y = -vec3.y;
            labelObj.location.z = -vec3.z;
            if (labelObj.imgPlaneMesh) {
                labelObj.imgPlaneMesh.position.set(labelObj.location.x, labelObj.location.y, labelObj.location.z);
                if (!labelObj.rotation) {
                    labelObj.imgPlaneMesh.lookAt(camera.position);
                }
                else {
                    labelObj.imgPlaneMesh.rotation.set(labelObj.rotation.x, labelObj.rotation.y, labelObj.rotation.z);
                }
            }
            if (labelObj.animationObj) {
                var mesh = labelObj.animationObj.getMesh();
                mesh.position.set(labelObj.location.x, labelObj.location.y, labelObj.location.z);
                if (!labelObj.rotation) {
                    mesh.lookAt(camera.position);
                }
                else {
                    mesh.rotation.set(labelObj.rotation.x, labelObj.rotation.y, labelObj.rotation.z);
                }
            }
        }
    };
    ...
    function enterNextScene() {
        var currentSceneIdx = opt.sceneGroup.indexOf(currentSceneObj);
        var nextSceneIdx = (currentSceneIdx + 1) % opt.sceneGroup.length;
        WebVR.enterScenebyId(opt.sceneGroup[nextSceneIdx].id);
    }
